...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    ...
    return [
      // Vercel 기본 도메인(*.vercel.app)으로 접속하면 정식 도메인으로 보낸다.
      // 검색엔진이 vercel.app 주소를 중복 색인해 sjac.co.kr 순위를 깎는 것을 막는다.
      {
        source: "/:path*",
        has: [{ type: "host" as const, value: "(?<sub>.*)\\.vercel\\.app" }],
        destination: "https://sjac.co.kr/:path*",
        permanent: true,
      },
      ...legacyPhpPages.map((page) => ({
        source: `/${page}.php`,
